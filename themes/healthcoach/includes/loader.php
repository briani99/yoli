...
add_action( 'init', 'healthcoach_bunch_theme_init');


if( !function_exists( 'healthcoach_set' ) ) {
	function healthcoach_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('healthcoach_printr' ) ) {
	function healthcoach_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('healthcoach__font_awesome' ) ) {
	function healthcoach_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = healthcoach_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('healthcoach_load_class' ) ) {
	function healthcoach_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
healthcoach_load_class( 'enqueue', 'helpers', false );

healthcoach_load_class( 'bootstrap_walker', 'helpers', false );


if( healthcoach_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function healthcoach_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include HEALTHCOACH_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include HEALTHCOACH_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = HEALTHCOACH_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = HEALTHCOACH_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( healthcoach_set( $_GET, 'page' ) == 'healthcoach'.'_option' ) 
		{
			if( healthcoach_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'healthcoach_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = HEALTHCOACH_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'healthcoach'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'healthcoach'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'healthcoach' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'healthcoach' ), // menu label
	));
