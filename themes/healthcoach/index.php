<?php healthcoach_bunch_global_variable(); 
	...
	if( $wp_query->is_posts_page ) {
		...
		$sidebar = healthcoach_set( $meta, 'sidebar', 'default-sidebar' );
		...
		if(healthcoach_set($_GET, 'layout_style')) $layout = healthcoach_set($_GET, 'layout_style'); else
		$layout = healthcoach_set( $settings, 'archive_page_layout', 'right' );
		$sidebar = healthcoach_set( $settings, 'archive_page_sidebar', 'default-sidebar' );
		$bg = healthcoach_set($settings, 'archive_page_header_img');
		$title = healthcoach_set($settings, 'archive_page_header_title');
		$text = healthcoach_set($settings, 'archive_page_header_text');
	}
	$layout = healthcoach_set( $_GET, 'layout' ) ? healthcoach_set( $_GET, 'layout' ) : $layout;
    ...
    <section class="page-title" <?php if($bg):?>style="background-image:url('<?php echo esc_attr($bg)?>');"<?php endif;?>>
        <div class="auto-container">
            <h1><?php if($title) echo wp_kses_post($title); else esc_html_e('Blog', 'healthcoach');?></h1>
